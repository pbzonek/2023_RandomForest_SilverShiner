## --------------------------------------------------------------#
## Script name: Script1-1-3_RF_Full_Adult.R 
##
## Purpose of script: 
##      Run RF model to predict Silver Shiner Occurrence
##      Full Features; Adult Life Stage
##
## Author: Paul Bzonek
##
...
RF_data_A %>% 
  group_by(Year, Month, maturep) %>% summarize(n=n())

#Split Train and Test data
set.seed(1758)
RF_data_A_train <- sample_frac(RF_data_A, 0.7) #Subset 70% for train
RF_data_A_test <-anti_join(RF_data_A, RF_data_A_train) #Remaining 30% for test
...
#----------------------------#
set.seed(1758)
RF_F_A_model <- randomForest(formula=maturep ~., data = RF_data_A_train, replace=TRUE, 
...
RF_data_A_test$prediction1 <- predict(RF_F_A_model, RF_data_A_test)

caret::confusionMatrix(
  data=RF_data_A_test$prediction1, 
  reference=RF_data_A_test$maturep,  
  positive="1")

RF_F_A_diagnostic <- list() #Start a Summary List
#Save and print confusion Matrix
RF_F_A_diagnostic$ConfusionMatrix <- 
  caret::confusionMatrix(
    data=RF_data_A_test$prediction1, 
    reference=RF_data_A_test$maturep,  
    positive="1",
...
RF_F_A_diagnostic$Summary <-
  data.frame(Accuracy = (RF_F_A_diagnostic$ConfusionMatrix[1]+RF_F_A_diagnostic$ConfusionMatrix[4])/sum(RF_F_A_diagnostic$ConfusionMatrix),
             AbsenceAccuracy = RF_F_A_diagnostic$ConfusionMatrix[1]/(RF_F_A_diagnostic$ConfusionMatrix[1]+RF_F_A_diagnostic$ConfusionMatrix[3]),
             PresenceAccuracy = RF_F_A_diagnostic$ConfusionMatrix[4]/(RF_F_A_diagnostic$ConfusionMatrix[4]+RF_F_A_diagnostic$ConfusionMatrix[2])) %>% 
  print()
...
#----------------------------#
###Evaluate RandomForest model
RF_data_A$pred.RF_F_A <- predict(RF_F_A_model, RF_data_A)
RF_F_A_CM <- caret::confusionMatrix(RF_data_A$pred.RF_F_A, RF_data_A$maturep, positive="1")

RF_F_A_VarImp <- data.frame(importance(RF_F_A_model)) %>% 
  mutate(predictor=rownames(.)) %>% 
  arrange(MeanDecreaseAccuracy)

#psuedo-R2 
RF_F_A_VarImp <- RF_F_A_VarImp  %>% 
...
RF_F_A_plot$R2 <- 
ggplot(RF_F_A_VarImp, aes(x=reorder(predictor, pR2), y=pR2))+
